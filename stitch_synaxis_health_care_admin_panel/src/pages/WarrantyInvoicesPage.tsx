...
export function WarrantyInvoicesPage() {
  ...
  const rows = data.filter((invoice: any) => (invoice.invoice_number + " " + invoice.customer_name_snapshot).toLowerCase().includes(search.trim().toLowerCase()))
  ...
  return <>
    ...
    <div className="glass-card rounded-xl p-4 sm:p-5">
      <label className="relative mb-5 block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant" size={18}/><input value={search} onChange={(event) => setSearch(event.target.value)} className="input-base w-full rounded-lg py-2.5 pl-10 pr-4" placeholder="Search invoice number or M/s..." /></label>
      ...
      <div className="overflow-x-auto max-md:overflow-visible">
        <table className="data-table mobile-card-table table-warranty-invoices">
          ...
          <tbody>{isLoading ? <tr><td colSpan={6}>Loading...</td></tr> : rows.length ? rows.map((invoice: any) => <tr key={invoice.id}>
          ...
          </tr>) : <tr><td colSpan={6} className="py-12 text-center"><FileCheck2 className="mx-auto mb-2"/>No warranty invoices match your search.</td></tr>}</tbody>
        </table>
      </div>
    </div>
  </>
