...
export type Doctor = { id: string; name: string; given_amount: number | string; percentage: number | string; required_business: number | string; entry_date: string; created_at: string; doctor_entries: { business: number | string }[] }
const doctorSchema = z.object({ entry_date: z.iso.date('Choose a valid date'), name: z.string().trim().min(1, 'Doctor name is required'), given_amount: z.number().min(0, 'Cannot be negative'), percentage: z.number().min(0, 'Cannot be negative').max(100, 'Cannot exceed 100%') })
...
export async function fetchDoctors() {
  const { data, error } = await supabase.from('doctors').select('id,name,given_amount,percentage,required_business,entry_date,created_at,doctor_entries(business)').order('name')
